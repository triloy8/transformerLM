    ...
    *,
    ...
    while True:
        ...
        if logger is not None:
            logger.log({"phase": "train", "metrics.loss": float(train_loss.item())}, step=train_iteration)
        ...
        if logger is not None:
            logger.log({"phase": "train", "metrics.grad_l2_norm": float(l2_norm.item())}, step=train_iteration)
        ...
        if train_iteration % val_freq_iteration == 0:
            ...
            if logger is not None:
                logger.log({"phase": "val", "metrics.loss": float(avg_val_loss)}, step=train_iteration)
